...
#../../cain.exe < Higgs4CP_"$JOBDIR".i > log_Higgs4CP_"$JOBDIR".txt 2>&1
#cd $CA/out/"$JOBDIR"
...
cd $CA/out/"$JOBDIR"
cp ../../in/HiggsIP_fullEcmRange.i HiggsIP_"$JOBDIR".i
../../cain.exe < HiggsIP_"$JOBDIR".i > log_HiggsIP_"$JOBDIR".txt 2>&1
cd $CA/out/"$JOBDIR"
