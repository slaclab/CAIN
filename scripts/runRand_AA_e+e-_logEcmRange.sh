...
cd $CA/out/"$JOBDIR"
cp ../../in/AA_e+e-_IP_logEcmRange.i AA_e+e-_IP_"$JOBDIR".i
sed -i "s!^ *SET Rand=.*!SET Rand=$RANDIP;!"  AA_e+e-_IP_"$JOBDIR".i
sed -i "s!xxxxxxxx!$JOBID!"  AA_e+e-_IP_"$JOBDIR".i
../../exec/cain.exe < AA_e+e-_IP_"$JOBDIR".i > log_AA_e+e-_IP_"$JOBDIR".txt 2>&1
cd $CA/out/"$JOBDIR"

mv AA_e+e-_IP.top AA_e+e-_IP_"$JOBDIR".top
td AA_e+e-_IP_"$JOBDIR".top POSTSCRIPT
$CA/ps2pdfRotate.sh AA_e+e-_IP_"$JOBDIR"
