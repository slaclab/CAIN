...
C  IRTN=-1 if the token is longer than LEN(TOKEN)
	...
	CHARACTER*1 CH
	character, parameter :: eorconst=achar(int(z'0A'))
	character, parameter :: eofconst=achar(int(z'00'))
c	CHARACTER*1 EOR/Z'0A'/,EOF/Z'00'/
	CHARACTER*1 EOR/eorconst/,EOF/eofconst/
	LOGICAL PEND/.FALSE./
	CHARACTER*1 LASTCH
	SAVE PEND,LASTCH
