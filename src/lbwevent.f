...
C     Input
C     N         ID of particle that scatters off the Laser (photon,  or parent electron/positron in case of virtual photon)
C     DT        time interval
...
C     IRTN      100: too many new particles in one time step.
 ...
 450  CONTINUE
      LBBF=LBBFIN(N)
      GEN2=GEN(N)+1
      write(msgfl,123) lenhbw,n,kind(n),lost(n),isbin(n),
     &     gen(n),wgt1,wgt(n),ep(0,n),ep(1:3,n)       
 123        format(' lbwevent lenhbw= ',i3,' n= ',i10,
     &           ' kind= ',i3,
     &           ' lost= ',i3,
     &           ' isbin= ',i4,
     &           ' gen= ',i4,
