...
C    Beam field on photon for coherent-pair calculation
 ...
 160      CONTINUE
          CALL BBKICK0(L,DS,1,TXYS(1,N),EP(0,N),BBFLD0,FF,IOUT,NXY,BBQ)
          CALL BBKICK0(L,DS,1,XYS1,EP(0,N),BBFLD,FF,IOUT,NXY,BBQ)
          DO 170 I=1,3
...
C-- Determine time step size
 ...
 230      CONTINUE
        CALL BBKICK0(L,DS,KIN,XYS1,EP1,BBFLD,FF,IOUT,NXY,BBQ)
        DO 240 I=1,3
...
C-- NDIV steps
     ...
     %       BBFL1,L,DS,KIN,NXY,BBQ)
        ELSE
C-- Runge-Kutta
          CALL RNGKTT(XYS0,EP0,DT,EXTFXY,
     %       BBFL1,ERMIN,BBDXY(1,L2),L,DS,KIN,NXY,BBQ,IRTN1)
        ENDIF
C--
        DO 620 I=1,3
