...
!  Read particle data from the file written by HiggsnCP.i
 BEAM  FILE='../out/higgs.dat';
!!
 SET Rand=123456789;
 SET MsgLevel=0;
 FLAG OFF ECHO;
 SET tmin=-ntcut*sigz, tmax=ntcut*sigz, nstep=300;
! SET tmin=-ntcut*sigz, tmax=ntcut*sigz, nstep=500;
!!
...
SET wmin=0.5*wmax;
 SET wbin=50;
!!
LUMINOSITY  KIND=(positron,electron), W=(wmin,wmax,wbin),
        WX=(8*sigx,16*sigx),WY=(10*sigy,16*sigy), FREP=nbunch*reprate ;
!LUMINOSITY  KIND=(photon,photon), W=(wmin,wmax,wbin);
!LUMINOSITY  KIND=(photon,electron), W=(wmin,wmax,wbin);
!LUMINOSITY  KIND=(electron,photon), W=(wmin,wmax,wbin);
!LUMINOSITY  KIND=(photon,positron), W=(wmin,wmax,50);
!LUMINOSITY  KIND=(positron,photon), W=(wmin,wmax,50);
!LUMINOSITY  KIND=(electron,positron), W=(wmin,wmax,50);
!LUMINOSITY  KIND=(electron,electron), W=(wmin,wmax,50);
...
!  Turn on incoherent pair
 ...
 PUSH  Time=(tmin,tmax,nstep) ;
    IF Mod(it,1)=0;
PRINT it, Time/1e-3, NParticle(1,3,'En>30e9'),  SigX(1,3,'En>30e9')/nm, FORMAT=(' +++ ',F6.0,'-th time step   t=',F7.2,' mm',' Right-going E> 30 GeV',' nPositrons=',E12.4,' SigX (nm)= ',E12.4);
PRINT it, Time/1e-3, NParticle(1,3,'En>60e9'),  SigX(1,3,'En>60e9')/nm, FORMAT=(' +++ ',F6.0,'-th time step   t=',F7.2,' mm',' Right-going E> 60 GeV',' nPositrons=',E12.4,' SigX (nm)= ',E12.4);
PRINT it, Time/1e-3, NParticle(2,2,'En>30e9'),  SigX(2,2,'En>30e9')/nm, FORMAT=(' +++ ',F6.0,'-th time step   t=',F7.2,' mm',' Left-going E> 30 GeV',' nElectrons=',E12.4,' SigX (nm)= ',E12.4);
PRINT it, Time/1e-3, NParticle(2,2,'En>60e9'),  SigX(2,2,'En>60e9')/nm, FORMAT=(' +++ ',F6.0,'-th time step   t=',F7.2,' mm',' Left-going E> 60 GeV',' nElectrons=',E12.4,' SigX (nm)= ',E12.4);
      PRINT STAT;
    ENDIF;
    IF it=Nint(nstep/2);
!!      PLOT  SCAT, KIND=electron, RIGHT, H=S/micron, V=Y/nm,
      PLOT  SCAT, KIND=electron, H=S/micron, V=Y/nm,
            FILE='../out/HiggsIP.top',
...
!        HTITLE='Q0e1 (mrad);GX X       ;' ;
!!
!PLOT LUMINOSITY, KIND=(photon,photon), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(photon,electron), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(electron,photon), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(electron,electron), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(photon,positron), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(positron,photon), FILE='../out/HiggsIP.top', APPEND;
!PLOT LUMINOSITY, KIND=(electron,positron), FILE='../out/HiggsIP.top', APPEND;
PLOT LUMINOSITY, KIND=(positron,electron), FILE='../out/HiggsIP.top', APPEND;
! 
 IF incpair >0;
...
PRINT Lum(3,2),FORMAT=('luminosity_e+e-=',1pd15.8);
! 
 STOP;
